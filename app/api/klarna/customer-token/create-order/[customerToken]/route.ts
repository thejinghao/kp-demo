...
export async function POST(
  ...
  try {
    ...
    const rawToken = params?.customerToken;
    const customerToken = Array.isArray(rawToken) ? rawToken[0] : rawToken;
    if (!customerToken || typeof customerToken !== 'string') {
      return NextResponse.json({ error: 'Missing customer token in path' }, { status: 400 });
    }

    const body = await req.json();

    const baseUrl = process.env.KLARNA_API_BASE_URL || 'https://api.playground.klarna.com';
    const url = `${baseUrl}/customer-token/v1/tokens/${encodeURIComponent(customerToken)}/order`;
